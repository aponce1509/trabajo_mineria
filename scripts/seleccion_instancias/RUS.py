...
Este código implementa el algoritmo de selección de instancias RandomUnderSampling
(RUS) a un conjunto de datos libre de valores perdidos. Para ello requiere de dos
archivos csv con la información del dataset inicial y de los paquetes pandas e 
...
rus.set_params(sampling_strategy='majority', replacement=True, random_state=42)
training_set_red, training_set_labels_red = rus.fit_resample(X_train, Y_train)
...
index = pd.DataFrame(rus.sample_indices_)
